...
export function setupWorkerSSR(app: Express) {
  app.use("/{*path}", async (req: Request, res: Response, next: NextFunction) => {
    const url = req.originalUrl;
    ...
    try {
      ...
      const templateResponse = await env.ASSETS.fetch(
        new Request(new URL("/index.html", `https://${req.headers.host || "running.services"}`)),
      );
      if (!templateResponse.ok) {
        throw new Error(`Unable to load the application shell (${templateResponse.status})`);
      }
      const template = await templateResponse.text();

      // Generated by script/build-cloudflare.ts before Wrangler bundles this Worker.
      // @ts-expect-error generated build artifact is intentionally excluded from tsc
